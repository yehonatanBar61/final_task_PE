...
class pe_extract:
    ...
    def __init__(self, file_path) -> None:
        ...
        # feature extraction 
        data = self.extract_infos(file_path)
        
        # matching it with the features saved in features.pkl
        pe_features = list(map(lambda x:data[x], features))
        print("Features used for classification: ", pe_features)
        
        self.res = clf.predict([pe_features])[0]
        # print ('The file %s is %s' % (os.path.basename(file_path),['malicious', 'legitimate'][res]))
...
# pe_peatures = pe_extract("documents/hwp5txt.exe")